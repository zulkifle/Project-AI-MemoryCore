...
- **`moomoo-api-trading`** (Lv.1.3) — executes/checks moomoo orders via futu-api + OpenD, derived from `Panduan_Moomoo_Claude_Code_2026.pdf`. Confirmed Bursa Malaysia (MY market) IS tradable via the API — verified directly from Dejul's own `TradeVault/moomoo-bridge/accounts.json` (security_firm=FUTUMY, market=MY/`TrdMarket.MY`), so the PDF's firm-scan step wasn't needed. Wired to consume position-size-calculator's Final Lots directly (×100 = order qty). Golden safety rules baked in as mandatory: SIMULATE by default, explicit confirm-before-REAL with full readback, respect stated RM cap, never expose the trading-unlock password, verify order status after every attempt. Dejul confirmed this stays standalone (Claude Code + futu-api direct) — NOT wired to `moomoo-bridge`, deferred for later.
- **Live-tested same session (2026-09-10)**: OpenD confirmed reachable (port 11111), Python/futu-api both installed. Ran real portfolio checks — SIMULATE account (acc_id `5181982`, CASH, default RM/USD 1,000,000, no positions) and REAL account (acc_id `286260077644734652`, MARGIN, total assets ~RM19,660, 2 open positions: MY.7233 DUFU +RM35, MY.0166 INARI -RM10). Found + fixed a real bug: `filter_trdmarket=TrdMarket.MY` on the trade context breaks SIMULATE queries (works fine for REAL) — must omit the market filter for SIMULATE calls. Confirmed real stock code format live: `MY.<Bursa numeric code>` (e.g. MY.7233, MY.0166). Confirmed request template (multi-line: code/EP/SL/ATR/star/Env). Confirmed order-type convention from Dejul's own manual trading: Entry = Limit (`OrderType.NORMAL`), SL = Stop (`OrderType.STOP`, `aux_price`=trigger) — resolves earlier uncertainty about SL semantics (a plain limit sell at SL price would've filled immediately, which was wrong).
- **Bug caught + fixed (2026-09-10)**: reported REAL `total_assets` as "RM19,659" without checking the `currency` field — it was actually `HKD` (base display currency of Dejul's multi-currency margin account), not MYR. Real MYR total is RM10,201.59 (`my_cash` + market value of MY positions), which Dejul caught by comparing against the app. Documented in the skill: always check `currency` before labeling amounts; prefer per-currency fields (`my_cash`, `hk_cash`, `us_cash`) over the base-currency top-level fields.
- Not yet placed an actual order (SIMULATE or REAL) through this skill — only read-only checks (snapshot/balance/positions) done so far.
